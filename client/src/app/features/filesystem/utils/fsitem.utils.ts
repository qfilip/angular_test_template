...
export class FsItemUtils {
  static mapRootFromEvents(events: FsItemEvent[], root?: FsItem) {
    ...
    const sorted = events.sort((a, b) =>
      Utils.timeSort(a.createdAt, b.createdAt)
    );
  ...
  static findAllPaths(item: FsItem) {
    ...
    return arr.reduce((acc, x) => {
      ...
      return last ? acc.concat(last + x) : [x];
    }, [] as string[]);
  }
  ...
  private static validateName(name: string, root: FsItem, parent: FsItem) {
    ...
    const duplicateNameCheck = () => {
      ...
      const sameNameItem = dds.dirs
        .concat(dds.docs)
        .find((x) => this.getName(x.path) === name);
  ...
  static doOnEvent<T>(
    ...
    switch (ev.type) {
      case 'created':
        return forCreated(ev as FsItemCreatedEvent);
      case 'updated':
        return forUpdated(ev as FsItemUpdatedEvent);
      case 'deleted':
        return forDeleted(ev as FsItemDeletedEvent);
      default:
        throw `Event of type ${ev.type} not handled`;
    }
  }
}
